    ...
    public partial class Service1 : ServiceBase
    {
        ...
        public void checkVehiclesInOutData()
        {
            try
            {
                ...
                MySqlCommand myComm = con.CreateCommand();
                MySqlDataReader Reader;
                myComm.CommandText = "SELECT id, vehiclenumber, indatetime, teirweight, teirweightdatetime, grossweight, grossdatetime, materialweight, po_number, lineweight, materialtype, challannumber, outdatetime, printdatetime, stageno FROM vehicleinoutlogs WHERE stagesync = 'Pending' ORDER BY id ASC";
                con.Open();
                Reader = myComm.ExecuteReader();
                while (Reader.Read())
                {
                    ...
                    {
                        // MessageBox.Show(Reader["id"].ToString());
                        updateInOutVehiclesInExcel(tripId, Reader["vehiclenumber"].ToString(), Reader["indatetime"].ToString(), Reader["teirweight"].ToString(), Reader["teirweightdatetime"].ToString(), Reader["grossweight"].ToString(), Reader["grossdatetime"].ToString(), Reader["materialweight"].ToString(), Reader["po_number"].ToString(), Reader["lineweight"].ToString(), Reader["materialtype"].ToString(), Reader["challannumber"].ToString(), Reader["outdatetime"].ToString(), Reader["printdatetime"].ToString(), Reader["stageno"].ToString());
                    }
                }
        ...
        public void updateInOutVehiclesInExcel(string tripId, string vehiclenumber, string indatetime, string tareweight, string tareweightdatetime, string grossweight, string grossweightdatetime, string materialweight, string po_number, string lineweight, string materialtype, string challannumber, string outdatetime, string printdatetime, string stageno)
        {
            String todayDate = DateTime.Now.ToString("yyyy MM dd");
            ...
            {
                // Create a file to write to.
                string createText = "Trip ID" + delimiter + "Vehicle Number" + delimiter + "In DateTime" + delimiter + "Tare Weight" + delimiter + "Tare Weight DateTime" + delimiter + "Gross Weight" + delimiter + "Gross Weight DateTime" + delimiter + "Material Weight" + delimiter + "PO Number" + delimiter + "Line" + delimiter + "Material Type" + delimiter + "Challan Number" + delimiter + "Out DateTime" + delimiter + "Print DateTime" + Environment.NewLine;
                File.WriteAllText(path, createText);
            }

            string appendText = tripId + delimiter + vehiclenumber + delimiter + indatetime + delimiter + tareweight + delimiter + tareweightdatetime + delimiter + grossweight + delimiter + grossweightdatetime + delimiter + materialweight + delimiter + po_number + delimiter + lineweight + delimiter + materialtype + delimiter + challannumber + delimiter + outdatetime + delimiter + printdatetime + delimiter + Environment.NewLine;
            File.AppendAllText(path, appendText);
