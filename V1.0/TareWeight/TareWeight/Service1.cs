...
namespace TareWeight
{
    public partial class Service1 : ServiceBase
    {
        ...
        String cmd = "";
        String T2VehicleNum = "", tripID = "", inDateTime = "", grossWeight = "", Retry_milisec = "";
        ...
        public void Validate_Trip_Entry(String vehicleNumber)
        {
            try
            {
                ...
                MySqlCommand myComm = con.CreateCommand();
                MySqlDataReader Reader;
                myComm.CommandText = "SELECT id, indatetime, grossweight FROM vehicleinoutlogs WHERE vehiclenumber = '" + vehicleNumber + "' ORDER BY id DESC LIMIT 1";
                con.Open();
                Reader = myComm.ExecuteReader();
                while (Reader.Read())
                {
                ...
                if ((!string.IsNullOrEmpty(tripID)) && (!string.IsNullOrEmpty(inDateTime)) && (!string.IsNullOrEmpty(vehicleNumber)) && (string.IsNullOrEmpty(grossWeight)))
                {
                    // Valid Entry
                    STW.WriteLine("|OPEN%");
