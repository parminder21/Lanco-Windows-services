...
namespace GrossWeight
{
    public partial class Service1 : ServiceBase
    {
                            ...
                            else if (cmd.StartsWith("|WBUSY-") && (cmd.EndsWith("%"))) // Vehicle Number command and weighing machine is busy
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        Insert_Voilation(vehicleNumber, "Weighing machine is busy, but some another vehicle comes near boom barrier."); // Insert voliation stage 3
                                    }
                                }
                                catch (Exception ex)
                                {
        ...
        public void Validate_Trip_Entry_Tare(String vehicleNumber)
        {
            try
            {
                ...
                else if(string.IsNullOrEmpty(inDateTime))
                {
                    ...
                    Insert_Voilation(vehicleNumber, "Tag is valid, but entry not found of this vehicle."); // Insert voliation stage 3
                }
                else if (string.IsNullOrEmpty(tareWeight))
                {
                    ...
                    Insert_Voilation(vehicleNumber, "Tag is valid, but tare weight not found of this vehicle."); // Insert voliation stage 3
                }
        ...
        public void Update_Logs_GrossWeight(string grossWeight, string tripID, string vehiclenumber)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(tareWeight) || (Int32.Parse(tareWeight) < 0))
                {
                    ...
                    Insert_Voilation(vehiclenumber, "Tare weight pending of this vehicle, but it was come on Gross weight."); // Insert voliation stage 3
                }
                else
                {
                    if (con == null)
        ...
        {
            var dateString = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            String DateTimeis = dateString, voilationStage = "3", ticketStatus = "OPEN";
