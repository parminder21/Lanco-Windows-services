...
namespace Entry
{
    public partial class Service1 : ServiceBase
    {
        ...
        /****************************** CHECK POLLU INS RC FITNESS Function *******************************/
        ...
        /****************************** CHECK Entry Exists OR Not Function ************** Check_TareWeight ****************************/
        public void Check_Entry_Exists_OR_Not(String vehicleNumber)
        {
            try
            {
                ...
                MySqlCommand myComm = con.CreateCommand();
                MySqlDataReader Reader;
                myComm.CommandText = "SELECT indatetime FROM vehicleinoutlogs WHERE vehiclenumber = '" + vehicleNumber + "' AND grossweight = '' AND outdatetime = '' ORDER BY id DESC LIMIT 1";
                con.Open();
                Reader = myComm.ExecuteReader();
                while (Reader.Read())
                {
        ...
        /****************************** CHECK THIS Vehicle TARE WEIGHT Function ************** |OPEN% *********** |BLINK% *****************/
        public void Check_TareWeight(String vehicleNumber, Boolean Entry_Available)
        {
            ...
            DateTime currentDatetime = DateTime.ParseExact(datetimeString, "yyyy-MM-dd HH:mm:ss", null);
            ...
            try
            {
                ...
                myComm.CommandText = "SELECT vehicleinoutlogs.teirweight, vehicleinoutlogs.teirweightdatetime from vehicleinoutlogs INNER JOIN vehicles ON vehicles.vehiclenumber = vehicleinoutlogs.vehiclenumber where vehicles.teirtype = 'Single' AND vehicleinoutlogs.vehiclenumber = '" + vehicleNumber + "' ORDER BY vehicleinoutlogs.id DESC LIMIT 1";
                con.Open();
                Reader = myComm.ExecuteReader();
                while (Reader.Read())
                {
                ...
                // If Tare weight and tare datetime is not empty. calculate hours using (previous tare datetime and current datetime)
                if ((!string.IsNullOrEmpty(tareWeight)) || (!string.IsNullOrEmpty(tareWeightDateTime)))
                {
                    // Calculate Houss
                    DateTime rcexpirydate_dtformat = DateTime.ParseExact(tareWeightDateTime, "yyyy-MM-dd HH:mm:ss", null);
                    TimeSpan rc_value = currentDatetime.Subtract(rcexpirydate_dtformat);
                    double tare_hours = rc_value.TotalHours;

                    if (tare_hours <= 23.999)
                    {
                        //Vehicle (Single Tare Found)
                        STW.WriteLine("|OPEN%");
                         if (DebugLog == true)
                         {
                            WriteToLogFile("Tare hours is:" + tare_hours);
                         }
                        Reader.Dispose();
                        myComm.Dispose();
                        con.Close();
