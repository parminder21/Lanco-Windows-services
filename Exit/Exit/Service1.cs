...
namespace Exit
{
    public partial class Service1 : ServiceBase
    {
        ...
        public static String Server4_IP = "";
        ...
        public void Update_Logs_ExitDetails(string vehiclenumber)
        {
            ...
            {
                con.Close();
                WriteToErrorFile("Gross Weight not found: " + ex.Message);
            }
            ...
            try
            {
                if (string.IsNullOrEmpty(grossWeight) || (Int32.Parse(grossWeight) < 0))
                {
                    ...
                    WriteToErrorFile("Gross Weight not found");
                    Insert_Voilation(vehiclenumber); // Insert voliation stage 4
                    Send_Audio_Message(vehicleNumber, "Authorise Vehicle. Go to gross weight.");
                }
                else if(string.IsNullOrEmpty(printdatetime))
                {
                    STW.WriteLine("|CHALLANPENDING%");
                    Send_Audio_Message(vehicleNumber, "Unauthorise Vehicle. Chalan is required.");
                }
                else
                {
                    string stageno = "4", stagesync = "Pending";
                    int materialweight = (Int32.Parse(grossWeight) - Int32.Parse(grossWeight));
                    string Query = "UPDATE vehicleinoutlogs SET outdatetime= '" + currentDate + "', stageno= '" + stageno + "', stagesync= '" + stagesync + "' WHERE vehiclenumber= '" + vehiclenumber + "' ORDER BY id DESC";
                    con = new MySqlConnection(DbPath.ToString());
        ...
        {
            var dateString = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
            String DateTimeis = dateString, voilationStage = "4", voilationMessage = "Gross weight pending of this vehicle, but it was come on exit gate.";
