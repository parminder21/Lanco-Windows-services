...
namespace GrossWeight
{
    public partial class Service1 : ServiceBase
    {
        ...
        public static String Server3_IP = "";
                        ...
                        if (cmd.Length > 0)
                        {
                            ...
                            else if (cmd.StartsWith("|WT-") && (cmd.EndsWith("%"))) // Vehicle Number command and weight 1 command
                            {
                                try
                                {
                                    ...
                                    if (cmd.Length > 0)
                                    {
                                        Update_Logs_GrossWeight(grossWeightis, T3VehicleNum);
                                    }
                                    else
                                    {
                                        STW.WriteLine("|WT-OK%");
        ...
        public void Update_Logs_GrossWeight(string grossWeight, string vehiclenumber)
        {
            var dateString = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            ...
            try
            {
                ...
                MySqlCommand myComm = con.CreateCommand();
                MySqlDataReader Reader;
                myComm.CommandText = "SELECT teirweight from vehicleinoutlogs where vehiclenumber= '" + vehiclenumber + "' ORDER BY id DESC LIMIT 1";
                con.Open();
                Reader = myComm.ExecuteReader();
                while (Reader.Read())
                {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string Query = "UPDATE vehicleinoutlogs SET grossweight= '" + grossWeight + "', grossdatetime= '" + currentDate + "', materialweight = '" + materialweight + "', stageno= '" + stageno + "', stagesync= '" + stagesync + "' WHERE vehiclenumber= '" + vehiclenumber + "' ORDER BY id DESC";
                    con = new MySqlConnection(DbPath.ToString());
